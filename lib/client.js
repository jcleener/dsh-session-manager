window.__ModuleLoader__.load({ id: 'dsh-session-manager', factory: (require) => {
  ...
  function Manager(props){
    ...
    const byId=sessionsSnap.byId||{}; const archived=new Set((workspacesSnap.archivedSessionIds||[])); const trash=new Set(Object.keys(meta.trash||{})); const baseIds=tab==='trash'?Object.keys(meta.trash||{}):(Array.isArray(sessionsSnap.ids)?sessionsSnap.ids:Object.keys(byId)); const isSub=s=>!!s&&(s.origin==='subagent'||s.parentId!==undefined); const items=baseIds.map(id=>{const s=byId[id]||{}; const m=meta.sessions[id]||{}; return {id,title:m.title||s.displayTitle||s.title||id,cwd:m.cwd||s.cwd||'',updated:s.updatedAt||0,archived:archived.has(id)||m.archived===true,trashed:trash.has(id),category:m.category||'',sub:isSub(s)};}).filter(x=>!x.sub&&(tab==='archived'?(x.archived&&!x.trashed):(tab==='trash'?x.trashed:!x.archived&&!x.trashed))&&(!q||((x.title+' '+x.cwd+' '+x.id).toLowerCase().includes(q.toLowerCase())))&&(!cat||(cat==='__none'?!x.category:x.category===cat)));
