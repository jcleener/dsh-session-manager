window.__ModuleLoader__.load({ id: 'dsh-session-manager', factory: (require) => {
  ...
  function Manager(props){
    ...
    const byId=sessionsSnap.byId||{}; const archived=new Set((workspacesSnap.archivedSessionIds||[])); const trash=new Set(Object.keys(meta.trash||{})); const baseIds=tab==='trash'?Object.keys(meta.trash||{}):(Array.isArray(sessionsSnap.ids)?sessionsSnap.ids:Object.keys(byId)); const items=baseIds.map(id=>{const s=byId[id]||{}; const m=meta.sessions[id]||{}; return {id,title:m.title||s.displayTitle||s.title||id,cwd:m.cwd||s.cwd||'',updated:s.updatedAt||0,archived:archived.has(id)||m.archived===true,trashed:trash.has(id),category:m.category||''};}).filter(x=>(tab==='archived'?(x.archived&&!x.trashed):(tab==='trash'?x.trashed:!x.archived&&!x.trashed))&&(!q||((x.title+' '+x.cwd+' '+x.id).toLowerCase().includes(q.toLowerCase())))&&(!cat||(cat==='__none'?!x.category:x.category===cat)));
    ...
    async function action(kind){const ids2=[...selected];if(kind!=='purge-all'&&!ids2.length)return;setBusy(true);setNotice('');setInfo('');let result=null;try{if(kind==='archive'){const w=workspacesApi;if(!w)throw new Error('会话工作区服务不可用');for(const id of ids2)await w.archiveSession(id);}else{const snaps={};if(kind==='trash'){for(const id of ids2){const it=items.find(x=>x.id===id);if(it)snaps[id]={title:it.title,cwd:it.cwd};}}const r=await fetch('/dsh-session-manager/metadata',{method:'POST',credentials:'same-origin',headers:{'content-type':'application/json'},body:JSON.stringify({ids:ids2,action:kind==='trash'?'trash':kind==='untrash'?'untrash':kind==='purge'?'purge':kind==='purge-all'?'purge-all':kind==='unarchive'?'unarchive':'categorize',category:newCat,sessions:snaps})});const x=result=await r.json().catch(()=>null);if(!r.ok||!x||!x.ok)throw new Error(x&&x.error?x.error:'操作失败');if(kind==='categorize')setNewCat('');}if(kind==='trash'||kind==='untrash'||kind==='unarchive'||kind==='purge'||kind==='purge-all'){const cur=sessionsSnap&&sessionsSnap.current;if(cur!==void 0&&ids2.includes(cur)){try{if(sessionsApi&&sessionsApi.clear)sessionsApi.clear();}catch(_e){}}try{await (sessionsApi&&sessionsApi.refresh?sessionsApi.refresh():Promise.resolve());}catch(_e){}try{await (workspacesApi&&workspacesApi.refresh?workspacesApi.refresh():Promise.resolve());}catch(_e){}}if(result&&Array.isArray(result.deferred)&&result.deferred.length)setInfo('已标记 '+result.deferred.length+' 个会话待删除：它们正在被使用，重启 DSH 后才会彻底清除。');setSelected(new Set());await load();}catch(e){setNotice(e&&e.message?e.message:'操作失败');}finally{setBusy(false)}}
